...
func (d *Domain) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		d.Log.Info().Str("collector", d.String()).Err(err).Msg("收集数据")
		success = 0
	}
	...
	for _, s := range stats {
		...
		if err != nil {
			d.Log.Info().Str("collector", d.String()).Str("desktop", id).Err(err).Msg("收集内存统计信息")
			log.Println(err)
		}
		...
		if err != nil {
			d.Log.Info().Str("collector", d.String()).Str("desktop", id).Err(err).Msg("收集桌面端口")
		}

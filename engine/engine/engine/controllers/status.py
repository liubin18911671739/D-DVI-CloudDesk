...
class UpdateStatus:
    def __init__(
        ...
        hostname,
        polling_interval,
        rate_allowed_diff_between_samples=2.5,
        port=22,
        user="root",
    ):
        self.hyp_id = id_hyp
        ...
        if type(self.hyp_obj) is hyp:
            try:
                ...
                try:
                    ...
                    log.info(
                        "reconnection to hypervisor {} in status thread fail".format(
                            self.hostname
                        )
                    )
            ...
            log.info(
                "unknown type, not hyp, reconnecting to hypervisor {} from status thread".format(
                    self.hostname
                )
            )
            try:
    ...
    def update_status_hyps_rethink(self):
        ...
        if self.try_connect_hyp() is False:
            log.error("#########################################################")
            log.error(
                "hypervisor {} connect fail in status thread".format(self.hostname)
            )
...
def launch_thread_status(hyp_id, polling_interval):
    t = ThreadStatus(
        name="status_" + hyp_id, hyp_id=hyp_id, polling_interval=polling_interval
    )
    t.daemon = True
    t.start()
    return t
